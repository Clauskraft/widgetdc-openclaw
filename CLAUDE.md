...
- Verify every action with a read-back, test, or render check (verification loops)
- Fix all failures and red tests before concluding a run (zero-tolerance)
- Reflect before returning: did this achieve the goal? Does output comply with contracts?
- Store top-1 improvement per area as `:ImprovementOpportunity` after each session
...
- End a session with unresolved failures or red tests
- Use ALL-CAPS enforcement language in prompts (calm language works better with frontier models)
